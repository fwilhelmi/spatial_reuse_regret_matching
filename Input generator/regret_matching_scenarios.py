...
def main():
    ...
    input_path = f"./input_constructor/input_template_random_dense_full.csv"
    ...
    for n in range(NUM_SCENARIOS):
        ...
        # for i in range(NUM_WLANS):
            ...
            for j in range(num_stas):
                ...
                print("      * d_min_AP_STA = ", d_min_AP_STA)
                print("      * d_max_AP_STA = ", d_max_AP_STA)
        ...
        for out_ix in range(len(backoff_approaches)):
            wlan_container = generate_wlans(backoff_approaches[out_ix], cw_adaptation, cw_min, cw_max, cw_stage,
                                            aps_position_list,
                                            stas_position_list)
            output_folder = f"./output/input_{NUM_WLANS}bss_fullbuffer/d_{map_width}/"
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
            output_path = output_folder+f"input_nodes_sce{n:02d}_FREQUENCY_REUSE_{FREQUENCY_REUSE}.csv"
